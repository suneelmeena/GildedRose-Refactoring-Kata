...
class GildedRose {
    ...
    private void updateItemQuality(Item item) {
        ...
        boolean doesDegrade = !item.name.equals(AGED_BRIE) && !item.name.equals(BACKSTAGE_PASSES_CONCERT) && !item.name.equals(SULFURAS_HAND);
        boolean hasSellByDate = !item.name.equals(SULFURAS_HAND);
    ...
    private int determineDegradeRate(Item item, boolean isExpired) {
        int baseDegradeRate = item.name.equals(CONJURED_MANA) ? -2 : -1;
